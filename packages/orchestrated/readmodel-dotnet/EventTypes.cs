...
public class EventConverter : JsonConverter<EventBase> {
  public override EventBase? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions? options) {
    using (JsonDocument jsonDoc = JsonDocument.ParseValue(ref reader)) {
      ...
      JsonElement eventText = root.GetProperty("event");
      string? type = eventText.GetProperty("type").GetString();

      switch (type) {
        case "CUSTOMER_CREATED":
          return JsonSerializer.Deserialize<CustomerCreatedEvent>(eventText.GetRawText(), options);
        case "CUSTOMER_UPDATED":
          return JsonSerializer.Deserialize<CustomerUpdatedEvent>(eventText.GetRawText(), options);
        case "ORDER_CREATED":
          return JsonSerializer.Deserialize<OrderCreatedEvent>(eventText.GetRawText(), options);
        case "ORDER_USD_RATE_AND_VALUE_ADDED":

          return JsonSerializer.Deserialize<OrderUsdRateAndValueAddedEvent>(eventText.GetRawText(), options);
        default:
