...
const keycloakUrl =
  import.meta.env.VITE_KEYCLOAK_URL || 'http://keycloak.localhost';
const keycloakRealm =
  import.meta.env.VITE_KEYCLOAK_REALM || 'lazyapps-demo';
const keycloakClientId =
  import.meta.env.VITE_KEYCLOAK_CLIENT_ID || 'lazyapps-frontend';
...
const startTokenRefresh = () => {
  ...
  refreshInterval = setInterval(() => {
    keycloak
      ...
      .then((refreshed) => {
        if (refreshed && onAuthStateChange) {
...
export const initAuth = (stateCallback) => {
  onAuthStateChange = stateCallback;
  return keycloak
    .init({
