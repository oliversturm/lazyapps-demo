<script>
	import { createMarked } from '$lib/marked-config.js';
	...
	const sendMessage = async () => {
		...
		try {
			...
			messages = [...messages, { role: 'assistant', type: 'error', content: error.message }];
		}
		loading = false;
...
<div class="flex flex-col h-[calc(100vh-120px)]">
	...
	<div class="flex items-center justify-between p-3 border-b">
		...
		<button class="text-xs px-2 py-1 rounded border hover:bg-gray-100" on:click={clearHistory}>
			Clear History
		</button>
	</div>
			...
			{:else}
				<div class="flex justify-start">
					<div class="bg-gray-50 rounded-lg px-4 py-2 max-w-xl">
						...
						<!-- Answer -->
						<div class="prose prose-sm">{@html marked.parse(message.content || '')}</div>
